...
exports.create = async (req, res) => {
  try {
    ...
    const { name, descriptionContent, detailContent, price, imageUploadUrl, inStock, categorySelected, tags, author, type, idSupplier } = req.body;
...
exports.getBySlug = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, data: { message: "Get product item success", product } });
  } catch (error) {
    console.error(error);
    res.status(404).json({ message: "FAIL", data: { message: error } });
  }
};
