...
exports.updateLanguage = async (req, res) => {
  try {
    const languages = await Setting.findOneAndUpdate({ _id: id }, { languages: req.body.languages });

    res.status(200).json({ success: true, message: "UPDATE LANGUE SUCCESS", languages });
  } catch (error) {
    console.log(error);
    res.status(404).json({ success: false, message: "UPDATE LANGUAGE FAILED" });
  }
};
